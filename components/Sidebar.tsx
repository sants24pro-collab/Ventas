...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, onExport }) => {
  const menu = [
    ...
    { id: View.AI_INSIGHTS, name: 'IA Insights', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
  ];
  ...
  return (
    <nav className="w-20 lg:w-72 bg-[#111111] flex flex-col items-center lg:items-stretch py-12 transition-all border-r border-[#222222] shadow-2xl">
      <div className="px-8 mb-20 flex items-center gap-4">
        ...
        <span className="hidden lg:block font-black text-2xl tracking-tighter text-[#F8F5F2] italic">ELITE STORE</span>
      </div>
        ...
        {menu.map(item => (
          <button
            ...
            className={`w-full flex items-center gap-5 px-6 py-5 rounded-full transition-all duration-500 group ${
              currentView === item.id 
                ? 'bg-[#F8F5F2] text-[#111111] font-black shadow-xl' 
                : 'text-[#555555] hover:text-[#F8F5F2] hover:bg-white/5'
            }`}
          >
            <svg className={`w-6 h-6 shrink-0 transition-transform duration-300 ${currentView === item.id ? '' : 'group-hover:scale-110'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={currentView === item.id ? 2.5 : 2} d={item.icon} />
            </svg>
            <span className="hidden lg:block text-xs uppercase font-black tracking-[0.2em]">{item.name}</span>
          </button>
        ))}
      </div>
      ...
      <div className="px-6 space-y-4 mt-auto">
        <button 
          onClick={onExport}
          className="w-full flex items-center gap-4 px-6 py-5 rounded-full bg-[#F8F5F2]/10 text-[#F8F5F2] border border-[#F8F5F2]/20 hover:bg-[#F8F5F2] hover:text-[#111111] transition-all group duration-300 shadow-lg"
        >
          <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          <span className="hidden lg:block text-[10px] font-black uppercase tracking-[0.2em]">Exportar Excel</span>
        </button>
      </div>
      ...
      <div className="px-8 mt-12 hidden lg:block opacity-40">
        ...
        <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
          ...
          <p className="text-xs font-bold text-[#F8F5F2]">Sincronizado</p>
        </div>
      </div>
    </nav>
  );
